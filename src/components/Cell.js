...
class Cell extends React.Component {
  ...
  /**
   * When a Cell value changes, re-determine the display value by calling the
   * formula calculation
   */
  onChange = e => {
  ...
  /**
   * Handle pressing a key when the Cell is a span element, not yet in editing
   * mode
   */
  onKeyPressOnSpan = () => {
  ...
  /**
   * Called by the `onBlur` or `onKeyPressOnInput` event handlers, it escalates
   * the value changed event, and restore the editing state to `false`
   */
  hasNewValue = value => {
  ...
  /**
   * Emits the `unselectedAll` event, used to tell all the other cells to unselect
   */
  ...
  /**
   * Performance lifesaver as the cell not touched by a change can decide to
   * avoid a rerender
   */
